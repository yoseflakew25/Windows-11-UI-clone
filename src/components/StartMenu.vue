<template>
    <div class="absolute flex flex-col overflow-hidden duration-200 bg-white rounded-md start-menu bg-opacity-80 dark:bg-gray-600 dark:bg-opacity-50 backdrop-filter backdrop-blur-lg" :class="{ 'show': isOpen }">
        ...
        <div class="flex flex-col flex-grow p-6">
            ...
            <div class="flex-grow">
                <div class="flex items-center px-6 py-4">
                    ...
                    <button class="flex items-center gap-2 px-3 py-1 duration-100 bg-white rounded-sm shadow-sm text-2xs hover:bg-gray-50 dark:hover:bg-opacity-20 dark:bg-opacity-10 dark:text-white">
                        <span>All apps</span>
                ...
                <div class="grid grid-cols-6">
                    <div v-for="(item, index) in apps" :key="index" class="flex flex-col items-center justify-center gap-1 py-2 duration-150 rounded-sm hover:bg-white hover:bg-opacity-50 active:bg-opacity-30 dark:hover:bg-opacity-10 dark:active:bg-opacity-5">
                        <img :src="item.icon" width="24"/>
                        <span class="font-medium text-center text-gray-800 text-2xs dark:text-gray-200">{{ item.label }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex-grow">
                ...
                <div class="grid grid-cols-2">
                    <div v-for="(item, index) in recommendedApps" :key="index" class="flex gap-3 px-6 py-3">
                        <img :src="item.icon" width="24"/>
                        <div class="flex flex-col">
                            <span class="font-medium text-gray-800 text-2xs dark:text-gray-200">{{ item.label }}</span>
                            <span class="font-medium text-gray-500 text-2xs dark:text-white dark:text-opacity-30">{{ item.subLabel }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="px-12 py-4 bg-black bg-opacity-5 dark:bg-opacity-10">
            <div class="flex items-center gap-2 user">
                <img src="../../name logo.jpg" class="rounded-full" width="26"/>
                <a href="https://github.com/YOSEF-CODER"><span class="text-gray-800 text-2xs dark:text-gray-300">Yosef Lakew</span></a>
                <div class="flex-grow"></div>
